...
public class CsvUtilsTest
{
	...
	@Test
	public void testFullMergeSortCsv2()
		throws Exception
	{
		// Here we split the original unordered file into ordered split files, then merge it back into an ordered result file
		// When we run checkCombinedSplit we need to use one as the combined file and the other as the single split file. It doesn't
		...
		Path combinedPath = dataDir.resolve("state-areas.csv");
		Path destPath = dataDir.resolve("temp_merged_state-areas.csv");
		Path[] destPaths = { destPath };
		int numRowLimit = 20;
		String columnHeader = "area sq mi";
	...
	@Test
	public void testFullMergeSortCsv3()
		throws Exception
	{
		// Here we split the original unordered file into ordered split files, then merge it back into an ordered result file
		// When we run checkCombinedSplit we need to use one as the combined file and the other as the single split file. It doesn't
		...
		Path combinedPath = dataDir.resolve("state-population.csv");
		Path destPath = dataDir.resolve("temp_merged_state-population.csv");
		Path[] destPaths = { destPath };
		int numRowLimit = 1000;
		String columnHeader = "population";
